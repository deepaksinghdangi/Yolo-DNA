...
class YoloSolver(Solver):
  ...
  def construct_graph(self):
    ...
    print("labels: {0} predicts: {1} object_num: {2}".format(str(self.labels.shape), str(self.predicts.shape),str(self.objects_num.shape)))
    
    #labels: Tensor of size: 16 x max_objects x 5(xcenter, ycenter, box_width, box_height, class_num)
    #objects_num: Tensor of size: 16 x max_object_per_image
    
    self.total_loss, self.nilboy = self.net.loss(self.predicts, self.labels, self.objects_num, self.sess)
