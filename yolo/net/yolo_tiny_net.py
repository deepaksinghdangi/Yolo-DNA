...
class YoloTinyNet(Net):
  ...
  def inference(self, images):
    ...
    temp_conv = self.max_pool(temp_conv, [2, 2], 2)
    #conv7 - no max pool now  7*7*1024
    ...
    # todo print self.cell_size * self.cell_size * (self.num_classes + self.boxes_per_cell(2) * 5)
    # shape(local3) = 7*7*(20+2*5)
    #16*(7*7*(20+2*5))

    n1 = self.cell_size * self.cell_size * self.num_classes # 7*7*20  = 980

    n2 = n1 + self.cell_size * self.cell_size * self.boxes_per_cell #7*7*20 + 7*7*2

    class_probs = tf.reshape(local3[:, 0:n1], (-1, self.cell_size, self.cell_size, self.num_classes))#16*7*7*20
    scales = tf.reshape(local3[:, n1:n2], (-1, self.cell_size, self.cell_size, self.boxes_per_cell))#16*7*7*2
    boxes = tf.reshape(local3[:, n2:], (-1, self.cell_size, self.cell_size, self.boxes_per_cell * 4))#16*7*7*(2*4)

    print("class_probs: {0} scales: {1} boxes: {2}".format(class_probs.shape, scales.shape, boxes.shape))
    local3 = tf.concat([class_probs, scales, boxes], 3) #16x7x7x30
  ...
  def body1(self, num, object_num, loss, predict, labels, nilboy):
    ...
    print("Mask image: "+str(objects.shape))
    #calculate objects  tensor [CELL_SIZE, CELL_SIZE]
    ...
    #print("Box dimensions: " + str(min_x.eval(session=sess)),str(min_y.eval(session=sess)))
    #print("center coordinates:" + str(center_x),str(center_y))
    ...
    #reshape: 7x7x8 -> 7x7x2x4
    predict_boxes = tf.reshape(predict_boxes, [self.cell_size, self.cell_size, self.boxes_per_cell, 4])

    #conversion of [xcenter,ycenter,bw,bh] into corresponding pixel values [64,64,448,448].
    #Note: xcenter, ycenter are in range [0,1]
    #Below operation will give the distance of xcenter and ycenter of each cell.
    ...
    #storing pixel value for each cell
    ...
    #base_boxes: 7x7x4
    #base_boxes conversion: 7x7x4 --> 7x7x1x4 
    #tile: 7x7x1x4 to 7x7x2x4
    ...
    #max value in I along 2nd dimension
    #max_I size: cell_size x cell_size x 1
  ...
  def loss(self, predicts, labels, objects_num, sess):
